...
var vm = new Vue({
  ...
  methods: {
    ...
    resizeMousedown(event){
      ...
      event.stopPropagation();
      event.preventDefault();
      console.log(366,event)
      this.resizeStatus = true;
      let box = event.target.parentNode;
      let fa = event.target.parentNode.parentNode
      let pos = {
        'w': box.offsetWidth,
        'h': box.offsetHeight,
        'x': event.clientX,
      ...
      this.resizeObj = {box,fa,pos}
      addEventHandler(document,'mousemove', this.resizeMousemove)
      addEventHandler(document,'mouseup', this.resizeMouseUp)
    },
    ...
    resizeMousemove(event) {
      ...
      let w = Math.max(0, event.clientX - pos.x + pos.w)
      let h = Math.max(0, event.clientY - pos.y + pos.h)
    ...
    resizeMouseUp(event) {
      console.log(2,event)
      removeEventHandler(document,'mousemove', this.resizeMousemove)
      removeEventHandler(document, "mouseup",this.resizeMouseUp);
    }
